...
def gain_link(movie_name):
    ...
    for i in range(len(r)):
        try:
            ...
            # 注意pop后需要挪位
            r[i-not_validate-fail]['link'] = resource[0]
            if resource[1] == []:
                r[i-not_validate-fail]['type'] = '未知'
            else:
                r[i-not_validate-fail]['type'] = resource[1][0]

            if resource[2] == []:
                r[i-not_validate-fail]['size'] = '未知'
            else:
                r[i-not_validate-fail]['size'] = resource[2][0]
